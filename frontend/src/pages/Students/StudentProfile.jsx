        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-8">
            ...
            <div className="bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group">
              <div className="absolute -bottom-6 -right-6 p-4 opacity-10 group-hover:rotate-12 group-hover:scale-110 transition-all">
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-4xl font-black italic flex items-baseline gap-1">
                      <span className="text-xl font-medium not-italic text-slate-500">₹</span>{totalFee.toLocaleString()}
                    </p>
                  </div>
                  <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden">
                    <div className="bg-gradient-to-r from-indigo-500 to-emerald-500 h-full" style={{ width: `${(paidAmount / (totalFee || 1)) * 100}%` }} />
                  </div>
                ...
                <tbody className="divide-y divide-slate-50">
                  {enrollments.flatMap(enr => enr.payment?.history || []).map((p, i) => (
                    <tr key={i} className="group hover:bg-slate-50">
                      <td className="py-4 font-bold text-slate-700 text-sm">{new Date(p.date).toLocaleDateString()}</td>
                      <td className="py-4 font-black text-indigo-600 text-sm">₹{p.amount.toLocaleString()}</td>
                      <td className="py-4 text-right text-[10px] font-bold text-slate-400">{p.method}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const FinanceItem = ({ label, value, color, align = "left" }) => (
  <div className={align === "right" ? "text-right" : ""}>
    ...
    <p className="text-xl font-bold text-white flex items-center gap-1 leading-none">
      <span className="text-sm font-normal text-slate-500">₹</span>{value.toLocaleString()}
    </p>
  </div>
);
...
const CourseCard = ({ enrollment }) => (
  <div className="group bg-slate-50 hover:bg-white p-6 rounded-3xl border border-transparent hover:border-slate-100 transition-all shadow-sm">
    <div className="flex justify-between items-start mb-4">
      <div className={`text-[10px] font-black px-3 py-1 rounded-full uppercase ${enrollment.status === 'ACTIVE' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
        {enrollment.status}
      </div>
...
const LoadingSkeleton = () => (
  <div className="h-screen w-full flex items-center justify-center bg-white">
    <div className="flex flex-col items-center gap-6">
      ...
      <p className="text-indigo-900 font-black text-xs tracking-[0.3em] uppercase">Initializing Portal</p>
    </div>
  </div>
);
...
const ErrorState = ({ message }) => (
  <div className="h-screen w-full flex items-center justify-center bg-slate-50 px-6">
    <div className="bg-white p-12 rounded-[3rem] shadow-2xl text-center max-w-sm border border-red-50">
      <div className="w-20 h-20 bg-red-50 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6"><ShieldCheck size={40} /></div>
      ...
      <button onClick={() => window.location.href = '/'} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs">GO TO LOGIN</button>
    </div>
  </div>
);
